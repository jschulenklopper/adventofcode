  ...
  pick_from_list(1,1,inventory[:weapons]) do |weapons|
    ...
    pick_from_list(0,1,inventory[:armor]) do |armor|
      ...
      pick_from_list(0,2,inventory[:rings]) do |rings|
        # Calculate total cost, just for convenience.
...
=begin
  total_cost = weapons.reduce(0) { |s,w| s += w[:cost] }
  total_cost += armor.reduce(0) { |s,w| s += w[:cost] }
  total_cost += rings.reduce(0) { |s,w| s += w[:cost] }
  end
=end
end
...
purchase_stuff(shop_inventory) do |cart|
  ...
  if me[:hitpoints] > 0
    ...
    lowest_cost = cost if lowest_cost == nil
    if cost < lowest_cost
      winning_cart = cart
      lowest_cost = cost
    end
